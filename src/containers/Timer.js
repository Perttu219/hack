...
const Timer = (props) => {
  ...
  useEffect(() => {
    if (state.seconds % 10 === 0) {
      fetch(
        "https://react-http-340ad-default-rtdb.europe-west1.firebasedatabase.app/timestamp.json",
        {
          method: "PATCH",
          body: JSON.stringify({
  ...
  return (
    <div className="counter">
      ...
      <p>
        Tiketti: <input type="text" onChange={inputHandler}></input>
      </p>
      <div>
      ...
      <button
        onClick={() =>
          dispatch(
            "FAKE",
            Math.floor(Math.random() * (19200000 - 1200000 + 1) + 1200000)
          )
        }
      >
        Fake timestamp
      ...
      <br />
      {state.timers.toString().split("|")[1]}
      <br />
      {state.timers.toString().split("|")[2]}
      <br />
      {state.timers.toString().split("|")[3]}
      <br />
      {state.timers.toString().split("|")[4]}
      <br />
      {state.timers.toString().split("|")[5]}
      <br />
      {state.timers.toString().split("|")[6]}
      <br />
      {state.timers.toString().split("|")[7]}
      <br />
      {state.timers.toString().split("|")[8]}
      <br />
      {state.timers.toString().split("|")[9]}
      <br />
      {state.timers.toString().split("|")[10]}
      <br />
      Current timeStamp in use: {state.timeStamp}
    </div>
  );
};
